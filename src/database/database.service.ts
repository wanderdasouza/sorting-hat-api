...
export class DatabaseService {
    ...
    async findAllServicesDatabases(serviceId: number): Promise<Store_At[]> {
        return await this.storeAtRepository.find({where: {service: {id: serviceId}}, relations: ['database']});
    }

    async create(databaseDto: DatabaseDto, serviceId: number, moduleId: number): Promise<Database> {
        const service = await this.serviceRepository.findOne(serviceId);
        const module = await this.moduleRepository.findOne(moduleId, {relations: ['databases']});
    ...
    async findAllModulesDatabases(moduleId: number) {
        return await (await this.moduleRepository.findOne(moduleId, {relations: ['databases']})).databases;
    }
