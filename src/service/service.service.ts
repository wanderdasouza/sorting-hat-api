...
export class ServiceService {
    ...
    async findAll(moduleId: number): Promise<Service[]> {
        const services = await this.serviceRepository.find({module: {id: moduleId}});
        return services;
    }
    ...
    async findOperations(serviceId: number): Promise<OperationService[]> {
        return this.operationsServiceRepository.find({service: {id: serviceId}});
    }

    async createOperation(operationServiceDto: OperationserviceDto, serviceId: number) {
    ...
    async createExternalResource(externalResourceDto: ExternalResourceDto, serviceId: number) {
        const service = await this.serviceRepository.findOne(serviceId);
    ...
    async findCommunications(serviceId: number) {
        return this.communicationRepository.find({service_emitter: {id: serviceId}});
    }

    async createCommunication(communicationDto: CommunicationDto, serviceId: number, receiverId: number) {
